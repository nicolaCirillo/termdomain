...
def wet_documents(pathsfile, reader, verbose=False, comm=None):
  ...
  with codecs.open(pathsfile, 'r', 'utf8') as f:
...
def corpus_documents(corpusfile, reader, verbose=False, comm=None):
  ...
  position = reader.position
  rank, size = ranksize(comm)
  with codecs.open(corpusfile, 'r', 'utf8') as f:
...
def load_vectors(vectorfile):
  ...
  words = set()
  with codecs.open(vectorfile, 'r', 'utf8') as f:
    for line in f:
...
def dump_vectors(generator, vectorfile):
  ...
  with codecs.open(vectorfile, 'w', 'utf8') as f:
...
def dump_targets(generator, targetfile):
  with codecs.open(targetfile, 'w', 'utf8') as f:
...
def main(args, comm=None):
  ...
  if args.targets is None:
    ...
    with codecs.open(args.targets, 'r', 'utf8') as f:
      if args.create_new:
